        ...
        if let mId = Int(managerIdInput), let mPass = Int(managerPassInput){
            if let tempManager: Manager = mda.getManager(id: mId){
                if tempManager._id == mId && tempManager._pin == mPass{
                    ...
                    let e3 = Employee(id: 3, firstName: "Elise", lastName: "Tang", pin: 1234, email: "et@georgebrown.ca", phone: "4161231234")
                    let e4 = Employee(id: 4, firstName: "Yuming", lastName: "Wu", pin: 1234, email: "yw@georgebrown.ca", phone: "4161231234")
                    let gm1 = GroupMember(id: 1, group: g1, employee: e1)
                    let gm2 = GroupMember(id: 2, group: g1, employee: e2)
                    let gm3 = GroupMember(id: 3, group: g2, employee: e3)
                    let gm4 = GroupMember(id: 4, group: g2, employee: e4)
                    let eda = EmployeeDA()
                    let gmda = GroupMemberDA()
                    eda.addEmployee(employee: e1)
                    eda.addEmployee(employee: e2)
                    eda.addEmployee(employee: e3)
                    eda.addEmployee(employee: e4)
                    gmda.addGroupMember(groupMember: gm1)
                    gmda.addGroupMember(groupMember: gm2)
                    gmda.addGroupMember(groupMember: gm3)
                    gmda.addGroupMember(groupMember: gm4)
